...
Seven executable gates cover provenance, revocation, partition boundaries,
event schema, Lindymode drift, RiverEditor shell commands, and L99 latency.
Each gate returns `(passed, reasons)` and `main()` exits non-zero on failure.
"""
...
TELEMETRY_SAMPLE_PATH = (
    ROOT / "samples" / "rivereditor_style_chain_telemetry.sample.json"
)
...
def gate_provenance() -> GateResult:
    ...
    for key in (
        ...
        if not value:
            reasons.append(
                f"policies/provenance_rules.json missing or empty required key: {key}"
            )
    ...
    with tempfile.TemporaryDirectory() as scratch:
        ...
        if clean["final_action"] != "allow":
            reasons.append(
                "clean decision input did not resolve to allow: "
                f"got {clean['final_action']}"
            )
        ...
        if boundary_violation["final_action"] != "block":
            reasons.append(
                "tenant-mismatched decision input did not resolve to block: "
                f"got {boundary_violation['final_action']}"
            )
        if not boundary_violation["incident_required"]:
            reasons.append(
                "boundary-violating decision did not set incident_required"
            )
...
def gate_revocation() -> GateResult:
    reasons: list[str] = []
    older = IsolationEnvelope.from_mapping(
        _base_envelope(revocation_epoch=7)
    )
    newer = IsolationEnvelope.from_mapping(
        _base_envelope(revocation_epoch=8)
    )
    if resolve_partition_id(older) == resolve_partition_id(newer):
        reasons.append(
            "partition_resolver produced the same partition id across "
            "different revocation epochs"
        )
    return (not reasons, reasons)
...
def gate_partition_boundary() -> GateResult:
    ...
    if resolve_partition_id(base) != resolve_partition_id(same):
        reasons.append(
            "identical isolation envelopes produced different partition ids"
        )
    ...
    for field in (
        ...
        varied = IsolationEnvelope.from_mapping(
            _base_envelope(**{field: "different_value"})
        )
        if resolve_partition_id(base) == resolve_partition_id(varied):
            reasons.append(
                f"varying required field '{field}' did not change the partition id"
            )
    return (not reasons, reasons)
...
def gate_event_schema() -> GateResult:
    ...
    for event in events:
        ...
        if problems:
            reasons.append(
                f"{event.get('event_id', '<unknown>')}: {'; '.join(problems)}"
            )
    return (not reasons, reasons)
...
def gate_lindymode_drift() -> GateResult:
    ...
    for event in events:
        ...
        if drift_score is None:
            reasons.append(
                f"{event_id}: state_drift_detected event is missing drift_score"
            )
            continue
        if not (0 <= drift_score <= 1):
            reasons.append(
                f"{event_id}: drift_score {drift_score} is outside [0, 1]"
            )
        ...
        if actual_severity not in {expected_severity, "sev3"}:
            reasons.append(
                f"{event_id}: drift_score {drift_score} implies severity >= "
                f"{expected_severity}, got {actual_severity}"
            )
    return (not reasons, reasons)
...
def gate_shell_command() -> GateResult:
    ...
    for entry in registry.get("commands", []):
        ...
        if key in seen:
            reasons.append(
                f"duplicate (mode, command) registry entry: {key}"
            )
        seen.add(key)
        ...
        if handler_name not in HANDLER_REGISTRY:
            reasons.append(
                f"command {entry.get('command')!r} references unregistered "
        ...
        try:
            ...
            reasons.append(
                f"command pattern {entry.get('command')!r} failed to "
                f"round-trip: {error}"
            )
            continue
        if resolved["handler"] != handler_name:
            reasons.append(
                f"command {entry.get('command')!r} resolved to handler "
                f"{resolved['handler']!r}, expected {handler_name!r}"
            )
    return (not reasons, reasons)
...
def gate_l99_latency() -> GateResult:
    ...
    if not records:
        return False, [f"no telemetry records found in {TELEMETRY_SAMPLE_PATH}"]
    report = build_report(records)
    summary = report["summary"]

    l99_latency = summary["l99_chain_latency_ms"]
    if l99_latency is None:
        reasons.append("L99 chain latency was not computed")
    elif l99_latency > L99_LATENCY_THRESHOLD_MS:
        reasons.append(
            f"L99 chain latency {l99_latency}ms exceeds threshold "
            f"{L99_LATENCY_THRESHOLD_MS}ms"
        )
    if summary["rollback_rate"] > ROLLBACK_RATE_THRESHOLD:
        reasons.append(
            f"rollback rate {summary['rollback_rate']} exceeds threshold "
            f"{ROLLBACK_RATE_THRESHOLD}"
        )
    if summary["validation_failure_rate"] > VALIDATION_FAILURE_RATE_THRESHOLD:
        reasons.append(
            "validation failure rate "
            f"{summary['validation_failure_rate']} exceeds threshold "
            f"{VALIDATION_FAILURE_RATE_THRESHOLD}"
        )
    return (not reasons, reasons)
