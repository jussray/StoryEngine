...
deliberately never executes anything (see its module docstring: "handlers are
named but not executed here"). This module is the runtime binding: it looks
up the resolved handler name, runs real handler logic for it, and emits the
command's declared events onto the shared L99 event bus.

Handler logic here is deterministic and dependency-free — there is no LLM
client in this repo, so `/ghost apply`, `/caveman rewrite`, and friends
produce structured, reproducible stand-in output rather than calling a model.
"""
...
from lindymode_event_emitter import LindymodeEvent, append_event as append_lindymode_event, emit_state_drift  # noqa: E402
...
CAVEMAN_DROP_WORDS = {"a", "an", "the", "of", "to", "in", "on", "at", "is", "are", "was", "were"}
...
def caveman_rewrite(text: str | None = None, **_kwargs: Any) -> dict[str, Any]:
    text = text or "She entered the room."
    words = [word for word in text.split() if word.strip(".,!?").lower() not in CAVEMAN_DROP_WORDS]
...
def open_episode(correlation_id: str, feed_path: str | Path = DEFAULT_FEED_PATH, **_kwargs: Any) -> dict[str, Any]:
    events = load_events(feed_path)
    return build_incident_artifact(correlation_id, events)


def show_events(feed_path: str | Path = DEFAULT_FEED_PATH, limit: int = 20, **_kwargs: Any) -> dict[str, Any]:
    events = load_events(feed_path)
    return {"event_count": len(events), "events": events[-limit:]}


def query_events(tenant_id: str, feed_path: str | Path = DEFAULT_FEED_PATH, **_kwargs: Any) -> dict[str, Any]:
    events = [event for event in load_events(feed_path) if event.get("tenant_id") == tenant_id]
...
def emit_lindymode_event(
    ...
    **kwargs: Any,
    ...
    severity = "sev2" if event_type in {"lindymode.continuity_conflict", "lindymode.context_budget_breach"} else "watch"
...
# --- Authoring / review stubs (registered, lightweight) --------------------
...
def summary_refresh(feed_path: str | Path = DEFAULT_FEED_PATH, tenant_id: str = DEFAULT_TENANT_ID, correlation_id: str | None = None, **_kwargs: Any) -> dict[str, Any]:
...
def audit_continuity(feed_path: str | Path = DEFAULT_FEED_PATH, tenant_id: str = DEFAULT_TENANT_ID, correlation_id: str | None = None, drift_score: float = 0.0, **_kwargs: Any) -> dict[str, Any]:
    ...
    return {"status": "audited", "drift_score": drift_score, "event_id": event["event_id"]}
...
def _emit_shell_event(*, event_type, tenant_id, correlation_id, reason, command, parent_event_id=None, status="completed", severity="info", feed_path):
    event = shell_event(
        event_type=event_type,
        tenant_id=tenant_id,
...
def dispatch(
    ...
    **handler_kwargs: Any,
    ...
    if handler is None:
        raise CommandNotFoundError(f"no handler registered for: {resolved['handler']}")
    ...
    # Precedence: shell-level context < command-pattern args (e.g. the
    # `<correlation_id>` in ":open episode <correlation_id>" means "the
    # incident to look up", which is a different value than the shell
    # command's own correlation_id) < explicit caller overrides.
    merged_args = {**context, **resolved["args"], **handler_kwargs}

    try:
        result = handler(**merged_args)
    except Exception as error:  # noqa: BLE001 - surfaced as a shell.command_failed event, then re-raised
        _emit_shell_event(
...
if __name__ == "__main__":
    ...
    # Demonstrate against a scratch copy of the sample feed so running this
    # self-test never mutates the checked-in samples/events.ndjson.
    with tempfile.TemporaryDirectory() as scratch_dir:
        ...
        for sample, extra_kwargs in [
            ...
            (":emit lindy-event", {"event_type": "lindymode.state_drift_detected", "reason": "selftest_drift", "drift_score": 0.6}),
            (":open episode inc_lindy_alpha_001", {}),
        ]:
            outcome = dispatch(sample, feed_path=scratch_feed, tenant_id="tenant_selftest", **extra_kwargs)
            print(sample, "->", json.dumps(outcome["result"], sort_keys=True)[:200])
