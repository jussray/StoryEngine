...
## StoryEngine proof boundary

StoryEngine's ordinary Node, promotion, Control Room, provider/runtime, and Guardrails Playwright evidence remain separate from continuity. `CURRENT` means only that the live base is an ancestor of the exact live head. Any successor SHA must earn StoryEngine's required machine and browser proof again before a merge or runtime claim.
