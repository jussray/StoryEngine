...
test('dedicated product controller identity is exact and cannot be widened by admin role alone', () => {
  assert.equal(isFounderControlRoomProductController({
    type: 'scoped_api_key',
    role: 'administrator',
...
test('invalid directives throw a bounded validation error while execution faults remain distinct', () => {
  const db = createEventDb();
  try {
    const invalid = validDirective();
    invalid.repository = 'jussray/not-storyengine';
    invalid.directiveHash = productBuildDirectiveHash(invalid);
    assert.throws(
      () => executeProductBuildDirective(db, invalid, { expectedHeadSha }),
      error => error instanceof ProductBuildDirectiveValidationError
        && error.errors.includes('product build directive repository mismatch'),
    );
  } finally {
    db.close();
  }
