...
function render(profile) {
  $('summary').innerHTML = `
    <h2>${esc(title(profile.medium))} for ${esc(title(profile.audience))}</h2>
  ...
  const entries = Object.entries(profile.resolved_rules || {}).filter(([key]) => ![
    'profile_instruction','require_human_decision','redteam_pre_runtime','redteam_pre_release'
  ].includes(key));
...
async function loadExisting() {
  ...
  try {
    ...
    $('summary').innerHTML = '<h2>Resolved Strategy</h2><p class="sub">No Creative Profile exists for this workspace yet.</p>';
    $('rules').innerHTML = '';
  }
}
...
$('profileForm').addEventListener('submit', async event => {
  ...
  $('status').textContent = 'Configuring L99…';
  try {
    ...
    render(profile);
    $('status').textContent = 'Creative Profile saved. OODA now uses this strategy.';
  } catch (error) {
