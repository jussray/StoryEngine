...
function computeWorkspaceSummary(db, windowMs) {
  ...
  return [...groups.values()].map(group => {
    ...
    const status = p99 > 2000 || rollbackRate > 0.05 || errorRate > 0.1
      ? 'critical'
      : p99 > 1000 || rollbackRate > 0.02 || errorRate > 0.05 ? 'warning' : 'healthy';
    return {
...
export function buildPerformanceDashboard(db, options = {}) {
  ...
  const totals = workspaces.reduce((acc, item) => {
    ...
    acc.p99_max = Math.max(acc.p99_max, item.p99);
    return acc;
  }, { events: 0, rollback_count: 0, error_count: 0, p99_max: 0 });
