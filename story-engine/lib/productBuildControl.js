...
export class ProductBuildDirectiveValidationError extends Error {
  constructor(errors) {
    super(errors.join('; '));
    this.name = 'ProductBuildDirectiveValidationError';
    this.errors = [...errors];
  }
}
...
export function isFounderControlRoomProductController(identity) {
  return identity?.type === 'scoped_api_key'
    && roleAtLeast(identity, 'administrator')
...
export function executeProductBuildDirective(db, directive, options = {}) {
  ...
  if (errors.length > 0) throw new ProductBuildDirectiveValidationError(errors);
